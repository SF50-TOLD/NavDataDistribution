...
struct ProcedureNameResolver: Sendable {
        ...
        switch record.kind {
          case .approach:
            ...
            for key in chart.keys where (best[key]?.penalty ?? .max) > chart.penalty {
              best[key] = (record.name, chart.penalty)
            }
  ...
  ///   - identifier: The CIFP procedure identifier, such as `"I28L"`.
  /// - Returns: The published title, or nil if no chart matches.
  func approachName(icao: String, approachType: ApproachType, identifier: String) -> String? {
    guard let key = ApproachKey(approachType: approachType, identifier: identifier) else {
      return nil
    }
    return approachCharts[icao]?[key]
  }
